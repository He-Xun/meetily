  ...
  return <>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex justify-between items-center p-6 border-b">
            <h3 className="text-xl font-semibold text-gray-900">{t('settingsModal.preferences')}</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div className="border-t pt-8">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.aiModelConfiguration')}</h4>
              <div className="space-y-4">
                ...
                {modelConfig.provider === 'ollama' && (
                  <div>
                    <h4 className="text-lg font-bold mb-4">{t('settings.availableOllamaModels')}</h4>
                    {error && (
                    ...
                    <div className="grid gap-4 max-h-[400px] overflow-y-auto pr-2">
                      {models.map((model) => (
                        ...
                        >
                          ...
                          <p className="text-gray-600">{t('common.size')}: {model.size}</p>
                          <p className="text-gray-600">{t('common.date')}: {model.modified}</p>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t('settingsModal.audioDeviceSettings')}</h3>
            <button
    ...
    {modals.languageSettings && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t('settingsModal.languageSettings')}</h3>
            <button
        ...
        <div className="bg-white rounded-lg max-w-4xl w-full mx-4 shadow-xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex justify-between items-center p-6 pb-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">
              {messages.modelSelector ? t('settings.speechRecognitionSetupRequired') : t('settings.transcriptionModelSettings')}
            </h3>
            <button
          ...
          <div className="p-6 pt-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700">{t('settingsModal.showConfidenceIndicators')}</p>
                <p className="text-xs text-gray-500">{t('settingsModal.confidenceIndicatorsDescription')}</p>
              </div>
            </div>
            ...
            >
              {messages.modelSelector ? t('settingsModal.cancel') : t('settingsModal.done')}
            </button>
          </div>
        </div>
      </div>
    ...
    {modals.errorAlert && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <Alert className="max-w-md mx-4 border-red-200 bg-white shadow-xl">
          <AlertTitle className="text-red-800">{t('settingsModal.recordingStopped')}</AlertTitle>
    ...
    {modals.chunkDropWarning && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <Alert className="max-w-lg mx-4 border-yellow-200 bg-white shadow-xl">
          <AlertTitle className="text-yellow-800">{t('settingsModal.transcriptionPerformanceWarning')}</AlertTitle>

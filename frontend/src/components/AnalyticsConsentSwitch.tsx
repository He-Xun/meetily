  ...
  return (
    <>
      <div className="space-y-4">
        <div>
          <h3 className="text-base font-semibold text-gray-800 mb-2">{t('analyticsConsent.title')}</h3>
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
          <div>
            <h4 className="font-semibold text-gray-800">{t('analytics.enableAnalytics')}</h4>
            <p className="text-sm text-gray-600">
              {isProcessing ? t('common.updating') : t('analytics.anonymousPatternsOnly')}
            </p>
          </div>
          ...
          <div className="p-4 border rounded-lg bg-gray-50">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 mb-1">{t('analyticsConsent.userIdTitle')}</div>
                <p className="text-xs text-gray-600 mb-2">
                  {t('analyticsConsent.userIdDescription')}
                </p>
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    title={t('analyticsConsentSwitch.copyUserId')}
                  >
                    {isCopied ? (
                      <>
                        <Check className="w-3.5 h-3.5 text-green-600" />
                        <span className="text-green-600">{t('analyticsConsentSwitch.copied')}</span>
                      </>
                    ) : (
                      <>
                        <Copy className="w-3.5 h-3.5" />
                        <span>{t('analyticsConsentSwitch.copy')}</span>
                      </>
                    )}
                  </Button>
                </div>
        ...
        <div className="flex items-start gap-2 p-2 bg-blue-50 rounded border border-blue-200">
          ...
          <div className="text-xs text-blue-700">
            ...
            >
              {t('analyticsConsent.viewPrivacyPolicy')}
            </button>
          </div>
        </div>
      </div>

        ...
        <button 
          ...
          className={`flex items-center justify-center cursor-pointer border-none transition-colors ${
            isCollapsed 
              ...
              : "w-full px-3 py-2 text-sm font-medium text-gray-700 bg-gray-200 hover:bg-gray-200 rounded-lg shadow-sm"
          }`}
          title={t('info.aboutMeetily')}
        >
          <InfoIcon className={`text-gray-600 ${isCollapsed ? "w-5 h-5" : "w-4 h-4"}`} />
          {!isCollapsed && (
            <span className="ml-2 text-sm text-gray-700">{t('common.about')}</span>
          )}
        </button>
      </DialogTrigger>
      <DialogContent>
        <VisuallyHidden>
          <DialogTitle>{t('info.aboutMeetily')}</DialogTitle>
        </VisuallyHidden>
        <About />
      </DialogContent>
    </Dialog>

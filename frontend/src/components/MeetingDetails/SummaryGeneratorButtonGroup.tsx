...
export function SummaryGeneratorButtonGroup({
  ...
  const checkBuiltInAIModelsAndGenerate = async () => {
    ...
    try {
      ...
      if (!selectedModel) {
        toast.error(t('summary.noModelConfigured'), {
          description: t('summary.selectModelToContinue'),
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (!modelInfo) {
        toast.error(t('models.modelNotReady'), {
          description: `${t('models.modelTaglineSmall')} - ${selectedModel}`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'downloading') {
        toast.info(t('models.downloadingModel'), {
          description: `${selectedModel} ${t('models.downloading')} (${status.progress}%). ${t('common.pleaseWait')}`,
          duration: 5000,
        });
        return;
      }
      ...
      if (status.type === 'not_downloaded') {
        toast.error(t('models.modelNotReady'), {
          description: `${selectedModel} ${t('models.downloadRequired')}. ${t('summary.openingModelSettings')}...`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'corrupted') {
        toast.error(t('models.modelCorrupted'), {
          description: `${selectedModel} ${t('models.fileCorrupted')}. ${t('models.pleaseRedownload')}`,
          duration: 7000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'error') {
        toast.error(t('models.modelError'), {
          description: status.Error || t('errors.unknownError'),
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      // Fallback
      toast.error(t('models.modelNotReady'), {
        description: t('summary.modelNotAvailableDescription'),
        duration: 5000,
      });
      setSettingsDialogOpen(true);
      ...
      toast.error(t('summary.failedToCheckModelStatus'), {
    ...
    try {
      ...
      if (isOllamaNotInstalledError(errorMessage)) {
        ...
        toast.error(
          ...
          {
            description: t('summary.ollamaInstallInstructions'),
            duration: 7000,
            action: {
        ...
        toast.error(
          t('summary.failedToCheckOllamaModels'),
          { duration: 5000 }
        );
      }
      setSettingsDialogOpen(true);
    ...
    <ButtonGroup>
      ...
      {isGenerating ? (
        ...
        >
          ...
          <span className="hidden lg:inline xl:inline">{t('common.stop')}</span>
        </Button>
      ) : (
        <Button
          variant="outline"
        ...
        >
          {isCheckingModels || isModelConfigLoading ? (
            ...
            <>
              ...
              <span className="hidden lg:inline xl:inline">{t('summary.generateSummary')}</span>
            </>
          )}
        </Button>
      )}
      ...
      <Dialog open={settingsDialogOpen} onOpenChange={setSettingsDialogOpen}>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            title={t('summaryGeneration.title')}
          >
            <Settings />
            <span className="hidden lg:inline">{t('meetingActions.aiModel')}</span>
          </Button>
        </DialogTrigger>
        <DialogContent
        ...
        >
          <VisuallyHidden>
            <DialogTitle>{t('modelManager.builtInAiModels')}</DialogTitle>
          </VisuallyHidden>
          <ModelSettingsModal
            onSave={async (config) => {
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              title={t('meetingActions.template')}
            >
              <FileText />
              <span className="hidden lg:inline">{t('meetingActions.template')}</span>
            </Button>
          </DropdownMenuTrigger>

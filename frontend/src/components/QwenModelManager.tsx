...
export function QwenModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        await invoke('qwen_init');

        // Get available models
        const modelList = await invoke('qwen_get_available_models') as QwenModelInfo[];
        setModels(modelList);

        setInitialized(true);
      } catch (err) {
        console.error('Failed to initialize Qwen:', err);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      console.log('[QwenModelManager] Setting up event listeners...');
      ...
      unlistenProgress = await listen<{ modelName: string; progress: number }>(
        ...
        (event) => {
          ...
          if (shouldUpdate) {
            console.log(`[QwenModelManager] Progress update for ${modelName}: ${progress}%`);
    ...
    return () => {
      console.log('[QwenModelManager] Cleaning up event listeners...');
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      await invoke('qwen_cancel_download', { modelName });
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      await invoke('qwen_delete_model', { modelName });
